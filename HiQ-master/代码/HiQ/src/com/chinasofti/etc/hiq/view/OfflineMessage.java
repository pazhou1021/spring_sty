...
public class OfflineMessage implements Runnable {
	...
	public OfflineMessage(Map<String, List<User>> usersMap) {
		super();
		this.usersMap = usersMap;
	}

	public OfflineMessage(Map<String, List<User>> usersMap, User userMy) {
		super();
		this.usersMap = usersMap;
		this.userMy = userMy;
	}
	...
	public void run() {
		...
		while(groupIterator.hasNext()) { // 遍历所有好友
			...
			while (userIterator.hasNext()) {
				...
				sendMsg = "offline/" + userMy.getUserQQ() + "/" + userMy.getUserNikName();
				try {
					if (user.getUserIP() != null) {
						...
						System.out.println("向" + userIP + "发送下线提醒");
					}
