...
//		int maxId = this.findMaxFriendsRsId();
		doOperate(sql, friendsRs.getUserQQ(),friendsRs.getFriendId(),friendsRs.getGroupName());
		closeAll();
		return result;
	}
	//根据用户QQ删除该用户的的好友关系
	...
	public int updateFriendsRsByfFriendId(int userQQ, int friendId,FriendsRs newFriendsRs) {
		// TODO Auto-generated method stub
		getConn();
		String sql = "update FriendsRss set userQQ = ?，friendId = ?,groupName = ? where userId = ?，friendId = ?";
		doOperate(sql, newFriendsRs.getUserQQ(),newFriendsRs.getFriendId(),newFriendsRs.getGroupName(),userQQ,friendId);
		closeAll();
		return result;
	}
